...
// jiangquan , enable mannual cofing for st7789v2
...
// 		WAIT;
	...
	// never reach
	while (1) {
		ESP_LOGI(TAG, "st7789 test task error");
...
{
	static uint32_t ledLoop = 0;

	gpio_set_direction(LED_GPIO, GPIO_MODE_OUTPUT);

	while(1) {
		gpio_set_level(LED_GPIO, (ledLoop++ % 2));
		ESP_LOGI(TAG, "Turning the LED %s!", ((ledLoop % 2) == true ? "ON" : "OFF"));
...
//	xTaskCreate(LedTaskEntry, "LedTaskEntry", 2048, NULL, 10, NULL);
//
//}
